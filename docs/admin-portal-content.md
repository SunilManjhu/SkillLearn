...
- **Disclosure widgets:** **Course details**, each **module**, and each **lesson** use chevron row headers with **`aria-expanded`**; modules and lessons follow an **accordion** (only one module and one lesson open at a time—see **`toggleModuleOpen`** / **`toggleLessonOpen`**). Full behavior and the Marketing-slides variant: [patterns-admin-disclosure-widgets.md](./patterns-admin-disclosure-widgets.md).
